...
async function waitForHealthyServer(maxRetries = 30) {
...
async function executeSmokeTestCases() {
  console.log("\n Test 1: GET /health");
  const healthRes = await request("http://localhost:4500/health");
  if (healthRes.statusCode !== 200) {
...
async function runTests() {
  ...
  try {
    ...
    const healthy = await waitForHealthyServer(30);
    if (!healthy) {
