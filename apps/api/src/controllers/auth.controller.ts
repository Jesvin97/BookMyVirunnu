...
import { AvailabilityRuleModel } from "../models/AvailabilityRule";
...
export class AuthController {
  ...
  quickRegister = asyncHandler(async (req: Request, res: Response) => {
    ...
    const event = await eventService.createEvent({
      ...
      // Created as draft to avoid a wasted slot sync before rules exist
      status: "draft",
      dietaryRestrictions: input.dietaryRestrictions || []
    });

    // Batch-insert all availability rules at once (no per-rule resync)
    const allDays = [0, 1, 2, 3, 4, 5, 6];
    const ruleDocs: Array<Record<string, unknown>> = [];
    ...
    if (ruleDocs.length > 0) {
      await AvailabilityRuleModel.insertMany(ruleDocs);
    }

    // Publish the event now — triggers a SINGLE slot sync with all rules in place
    await eventService.publishEvent(event._id.toString(), user._id.toString());
