...
export function createApp() {
  ...
  const getCorsOrigin = (): string | string[] | boolean | ((origin: string | undefined, callback: (err: Error | null, allow?: boolean) => void) => void) => {
    ...
    const whitelist = new Set(env.corsOrigin.split(",").map((o) => o.trim().replace(/\/$/, "")));
    return (origin, callback) => {
      const sanitizedOrigin = origin ? origin.replace(/\/$/, "") : undefined;
      if (!sanitizedOrigin || whitelist.has(sanitizedOrigin)) {
        callback(null, true);
      } else {
  ...
  app.get("/", (_req, res) => {
    res.status(200).send(
      "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\" /><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\" /><title>BookOurVirunnu API</title></head><body style=\"font-family:system-ui,sans-serif;padding:24px;\"><h1>BookOurVirunnu API</h1><p>The API is running. Health check: <a href=\"/health\">/health</a></p></body></html>"
    );
  });
