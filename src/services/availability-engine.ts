...
import type { AvailabilitySlot, SlotConflict } from "../types";
import {
  addMinutes,
  compareDatesAsc,
  formatDateKey,
...
export class AvailabilityEngine {
  ...
  async syncSlotsForRange(input: BuildRange): Promise<void> {
    ...
    if (operations.length > 0) {
      await BookingSlotModel.bulkWrite(operations, {
        ordered: false,
  ...
  validateRequestedSlot(input: {
    ...
    if (event.status !== "published") {
      return {
        ...
        message: "This event is not accepting bookings right now."
      };
    }
    ...
    if (requestedStartAt.getTime() < addMinutes(now, event.bookingRules.minLeadMinutes).getTime()) {
      return {
        ...
        message: "This slot is too close to the current time to book."
      };
    }

    const eventStart = event.startDate.getTime();
    const eventEnd = event.endDate.getTime();
    if (requestedStartAt.getTime() < eventStart || requestedEndAt.getTime() > eventEnd) {
      return {
        code: "outside_event_window",
    ...
    if (slot.status === "blocked") {
      return {
        code: "blackout_rule",
        message: "This slot is blocked by host availability rules."
      };
    }
    ...
    if (slot.status === "locked" && slot.reservedCount < slot.capacity) {
      return {
        code: "slot_locked",
        message: "This slot is locked by the host."
      };
    }
    ...
    const blockedMatch = rules.find((rule) => {
      ...
      return this.ruleAppliesToDate(rule, requestedStartAt, event.timezone) && this.intervalsOverlap(
        {
          start: slot.occupiedStartAt,
          end: slot.occupiedEndAt
        },
        {
  ...
  private computeOpenIntervals(dateKey: string, rules: AvailabilityRuleDocument[], timezone: string): Interval[] {
    ...
    const mergedOpen = this.mergeIntervals(openIntervals);
    const cleaned = this.subtractIntervals(mergedOpen, blockedIntervals);
  ...
  private subtractIntervals(open: Interval[], blocked: Interval[]): Interval[] {
    ...
    for (const interval of open) {
      ...
      for (const block of sortedBlocked) {
        if (!this.intervalsOverlap({ start: cursorStart, end: interval.end }, block)) {
          continue;
        }
