...
export class BookingService {
  ...
  async createBooking(input: CreateBookingInput): Promise<BookingDocument> {
    ...
    try {
      ...
      await session.withTransaction(async () => {
        ...
        if (conflict) {
          throw new BookingConflictError(conflict.code, conflict.message, conflict.details);
        }
        ...
        if (existingActive) {
          throw new BookingConflictError(
            "duplicate_booking",
            "This guest already has an active booking for the event.",
      ...
      if (!createdBooking) {
        throw new Error("Booking transaction completed without creating a booking.");
      }

      return createdBooking;
    } catch (error) {
      if (error instanceof BookingConflictError || error instanceof NotFoundError) {
        throw error;
      }
      ...
      if (mongoError?.code === 11000) {
        ...
        throw new BookingConflictError(
          "duplicate_booking",
          "A conflicting booking already exists for this guest and event."
        );
      }
  ...
  async listMyBookings(guestUserId: string): Promise<BookingDocument[]> {
    return BookingModel.find({
      guestUserId: new Types.ObjectId(guestUserId)
    })
      .sort({ createdAt: -1 })
      .exec();
  }
  ...
  private assertBookingShape(event: EventDocument, startAt: Date, endAt: Date, partySize: number): void {
    ...
    if (duration !== event.bookingRules.slotDurationMinutes) {
      throw new BookingConflictError(
        ...
        "Bookings must match the configured slot duration exactly.",
        { expectedMinutes: event.bookingRules.slotDurationMinutes, actualMinutes: duration }
      );
    }
    ...
    if (partySize > event.bookingRules.maxGuestsPerSlot) {
      throw new BookingConflictError(
        "capacity_limit",
        "The requested party size exceeds the maximum guest capacity for a slot.",
